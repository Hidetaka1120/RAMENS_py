...
def dacget4s(file):
  ...
  return data.reshape(mdim[4],mdim[3],mdim[2],mdim[1],mdim[0]), mdim[0], mdim[1], mdim[2], mdim[3]



def dac_read4d(filename):
  ...
  dir = os.path.dirname(os.path.abspath(flist[0]))+'/'
  ix  = dacgetparam(dir+'params.0000.txt','ix')
  jx  = dacgetparam(dir+'params.0000.txt','jx')
  kx  = dacgetparam(dir+'params.0000.txt','kx')
  margin = dacgetparam(dir+'params.0000.txt','margin')
  ipex = dacgetparam(dir+'params.0000.txt','ipex')
  jpex = dacgetparam(dir+'params.0000.txt','jpex')
  kpex = dacgetparam(dir+'params.0000.txt','kpex')
  mpex = dacgetparam(dir+'params.0000.txt','mpex')

  igx=ix*ipex-(1+2*margin)*(ipex-1)
  jgx=jx*jpex-(1+2*margin)*(jpex-1)
  kgx=kx*kpex-(1+2*margin)*(kpex-1)
...
# read x,y,z axes
  mpirank=0
  for mz in range(1,kpex+1):
    for my in range(1,jpex+1):
      for mx in range(1,ipex+1):
        file = dir+'x.'+'{:04d}'.format(mpirank)+'.dac'
        xd = dacget1d(file)
        file = dir+'y.'+'{:04d}'.format(mpirank)+'.dac'
        yd = dacget1d(file)
        file = dir+'z.'+'{:04d}'.format(mpirank)+'.dac'
        zd = dacget1d(file)
        ixs = (mx-1)*ix-(1+2*margin)*(mx-1)
        ixe = ixs+ix-1
        jxs = (my-1)*jx-(1+2*margin)*(my-1)
        jxe = jxs+jx-1
        kxs = (mz-1)*kx-(1+2*margin)*(mz-1)
        kxe = kxs+kx-1
        x[ixs:ixe+1] = xd
        y[jxs:jxe+1] = yd
        z[kxs:kxe+1] = zd
  ...
  nd = int(len(flist)/nproc)
  # print(mpex)
  data = np.zeros([nd,32,kgx,jgx,igx])
  ...
  for l in range(0,nd):
    ...
    for mz in range(1,kpex+1):
      for my in range(1,jpex+1):
        for mx in range(1,ipex+1):
          temp, ix, jx, kx, mpex = dacget4s(flist[l*nproc+mpirank])
          ixs = (mx-1)*ix-(1+2*margin)*(mx-1)
          ixe = ixs+ix-1
          jxs = (my-1)*jx-(1+2*margin)*(my-1)
          jxe = jxs+jx-1
